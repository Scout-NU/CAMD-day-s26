...
function speakerCard(s) {
  return `
    <div class="flex flex-col items-start w-full min-w-0">
      ...
      <p class="mt-4 text-xl font-semibold tracking-wide">${s.name}</p>
    </div>
  `;
}
...
function speakerSlide(speakersInSlide) {
  return `
    <div class="speaker-slide shrink-0 overflow-hidden grid grid-cols-2 lg:grid-cols-4 gap-10">
...
export function render() {
  ...
  return `
    ...
    >
      <div class="flex flex-row justify-between items-start px-12">
        <h1 class="text-5xl sm:text-7xl lg:text-8xl xl:text-[110px] font-medium leading-tight">
          FEATURED SPEAKERS
        </h1>
        <div class="flex items-center gap-3 pt-4 pr-2">
          ${dotsHtml}
        </div>
      </div>
